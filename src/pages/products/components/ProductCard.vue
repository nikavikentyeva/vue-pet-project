<template>
  <div class="bg-white h-full rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 relative">
    <a href="#">
      <img class="p-8 h-96 rounded-t-lg" :src="item.mainImage" alt="product image" />
    </a>
    <div class="px-5 pb-5">
      <div class="uppercase flex justify-end">{{ item.category }}</div>
      <div @click="getSingle(item)" class="cursor-pointer">
...
async function getSingle (item) {
  let data = await ProductService.getSingle(item.id)
  alert (data.title)
}
</script>
